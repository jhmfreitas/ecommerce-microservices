...
public class OrderControllerWebMvcTest {
  ...
  public void getOrderSuccess() throws Exception {
    ...
    mockMvc.perform(get("/order")).andExpect(status().isOk())
        ...
        .andExpect(jsonPath("$.content[0].orderDate").value(now.toString()))
        .andExpect(jsonPath("$.content[0].items[0].quantity").value(quantity))
        .andExpect(jsonPath("$.content[0].items[0].productId").value(productId))
        .andExpect(jsonPath("$.content[0].items[0].productName").value(productName))
        .andExpect(jsonPath("$.content[0].items[0].unitPrice").value(unitPrice));
    ;
  }
}
